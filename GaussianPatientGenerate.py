...
Kernels = ({'t': [1000, 0.001, 10000], 'ut': [20, 0.001, 2000], 'Pt': [0.02, 0.000001, 20], 'Gt': [0.5, 0.001, 10], 'SIt': [1, 0.001, 1000]})
Features = ['t', 'ut', 'Pt']
Kernel_Width = []
Kernel_Bound = []
...
for i in range(1,1669):
    ...
    #Kernel_Result= pd.DataFrame({'k_SIt': RBF_Params.length_scale[0], 'k_Gt': RBF_Params.length_scale[1], 'Score': Score, 'Gt': np.mean(GlucData['Gt']), 'SI': np.mean(GlucData['SIt']), 't': t[-1], 'Gender': GlucData['Gender'].values[0], 'Operative': GlucData['Operative'].values[0]}, index = [i])
    Kernel_Result= pd.DataFrame({'k_constant': GPG.kernel_.get_params()['k1__constant_value'], 'k_SIt': RBF_Params.length_scale[0], 'k_ut': RBF_Params.length_scale[1], 'k_Pt': RBF_Params.length_scale[2], 'Score': Score, 'Gt': np.mean(GlucData['Gt']), 'SI': np.mean(GlucData['SIt']), 't': t[-1], 'Gender': GlucData['Gender'].values[0], 'Operative': GlucData['Operative'].values[0]}, index = [i])
